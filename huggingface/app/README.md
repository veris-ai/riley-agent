...
| Module | Role |
|--------|------|
| `main.py` | The whole agent process. A FastAPI app exposing `WS /voice` (and `/health`); each connection runs the cascade in `_Call` — VAD, utterance buffering, and endpointing on the caller's audio, one Whisper request per utterance, an LLM chat completion with the five tools, and Kokoro TTS, paced back out against playback. Each leg is an HF-hosted call: a dedicated Inference Endpoint when its `HF_*_URL` is set (the benchmarked configuration), the serverless router otherwise. |
...
4. The worker downsamples the utterance to 16 kHz, wraps it in a WAV, and
   POSTs it to the Whisper leg. A blank transcript ends the turn quietly.
5. The transcript joins `messages` and the worker walks the tool loop: a chat
   completion against the LLM leg → `dispatch()` against `BCSAPI` for each
   tool call (reported via `reporting.py`) → repeat until the model answers
   in text.
6. The reply goes to the Kokoro leg, which returns a WAV; the worker converts
   it to PCM16 @ 24 kHz and sends it in 0.5 s chunks, throttled to stay ~1 s
   ahead of real-time playback.
