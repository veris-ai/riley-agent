...
it lands in the graded trace. The Voice Agent session runs the LLM in
Deepgram's cloud and the tools execute in-process here, so tool calls never
reach the actor transcript; without this the grader can't see them and may flag
real, completed actions as fabricated. ``SIMULATION_ID`` and ``ENGINE_URL`` are
set by the sandbox; outside a simulation this no-ops.
"""
...
logger = logging.getLogger("riley-deepgram")
...
def report_tool_call(name: str, args: dict, result: object) -> None:
    ...
    never stalls the realtime voice loop, even if the engine is slow/unreachable.
    """
