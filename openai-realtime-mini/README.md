# riley-openai-realtime

Riley, a card-support voice agent for Acme Bank, built directly on the **raw [OpenAI Realtime API](https://platform.openai.com/docs/guides/realtime)** — a single speech-to-speech model (`gpt-realtime-2.1-mini`), no STT/LLM/TTS cascade. Riley handles credit-card replacement and status-update calls end to end, backed by five Postgres tools. A single FastAPI process exposes the `voice_ws` endpoint (raw PCM16 over a plain WebSocket) and, per call, opens its own Realtime session and bridges audio and tool calls between the caller and OpenAI.

## What it does

One process runs inside the container (`uvicorn app.main:app` on `:8008`):

- **`app/main.py`** — a FastAPI app exposing `WS /voice` (plus `/health`). Each `/voice` connection opens a dedicated OpenAI Realtime WebSocket, configures the session (instructions from `agent_desc.txt`, the five card-ops tools, `server_vad` with 800 ms end-of-turn silence), sends an initial `response.create` so Riley greets first, then runs two pumps: caller PCM16 → Realtime input buffer, and Realtime events → caller audio + tool dispatch.
...
flowchart LR
    ...
    oa["OpenAI Realtime<br/>gpt-realtime-2.1-mini"]

    caller <-->|"PCM16 WS"| bridge
    bridge <-->|"Realtime WS<br/>(audio + tool events)"| oa
    bridge -->|"tool calls"| pg
```

Unlike a cascaded pipeline, the LLM, STT, and TTS all live inside OpenAI's Realtime session — the container only moves audio bytes and executes tool calls locally against Postgres.
...
ENV_ID=$(curl -sS -X POST "$VERIS_URL/v1/environments" \
  ...
  -d "{\"name\":\"riley-openai-realtime\",\"organization_id\":\"$VERIS_ORG\",\"skip_managed_onboarding\":true}" \
  | jq -r '.environment.id')
echo "$ENV_ID"
```
...
```bash
veris env vars set OPENAI_API_KEY=sk-... --secret --env-id "$ENV_ID"
```
...
```bash
docker build --platform linux/amd64 -f Dockerfile.bench -t <registry>/riley-openai-realtime:v1 .
docker push <registry>/riley-openai-realtime:v1
```

Import `.veris/veris.yaml` as a managed image candidate, set `image.path` to
`/voice` and `image.health_path` to `/health`, and add `OPENAI_API_KEY` as a secret
candidate environment entry. The benchmark engine stamps the candidate's
in-cluster address into the `voice_ws` channel.
...
| Channels      | mono                                  |
| Frame size    | 20 ms (960 bytes) in; out streams Realtime's audio deltas as they arrive |
...
|------------------|----------|------------------|-------|
| `OPENAI_API_KEY` | yes      | —                | the Realtime session |
| `DATABASE_URL`   | yes      | (set by Veris)   | Postgres for the card-ops tools |
| `REALTIME_MODEL` | no       | `gpt-realtime-2.1-mini` | Realtime model override |
| `REALTIME_VOICE` | no       | `alloy`          | Realtime voice |
