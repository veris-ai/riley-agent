...
The LLM leg is any OpenAI-compatible chat-completions endpoint, which takes
tools in the OpenAI function-calling shape — `{"type": "function", "function":
{name, description, parameters}}` — so the schemas are declared bare here and
wrapped once at import. `dispatch()` maps a tool call to the corresponding
`BCSAPI` operation and returns a JSON-serializable result.
"""
