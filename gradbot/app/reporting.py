...
it lands in the graded trace. gradbot runs the LLM against an OpenAI-compatible
API and hands tool calls back to this process, so they execute in-process and
never reach the actor transcript; without this the grader can't see them and
may flag real, completed actions as fabricated. ``SIMULATION_ID`` and
...
logger = logging.getLogger("riley-gradbot")
