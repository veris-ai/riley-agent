...
it lands in the graded trace. Gemini Live runs the LLM in Google's cloud and
the tools execute in-process here, so tool calls never reach the actor
transcript; without this the grader can't
see them and may flag real, completed actions as fabricated. ``SIMULATION_ID``
...
logger = logging.getLogger("riley-gemini-live")
