...
The schemas are Gemini Live function declarations — one ``function_declarations``
block passed in ``LiveConnectConfig.tools``. Unlike the OpenAI Realtime wire
format there is no ``{"type": "function"}`` wrapper and no ``additionalProperties``
key (Gemini uses an OpenAPI 3.0 subset for ``parameters``). ``dispatch()`` maps a
Gemini ``FunctionCall`` to the corresponding ``BCSAPI`` operation and returns a
...
# Single tools entry for LiveConnectConfig.tools.
TOOLS: list[Dict[str, Any]] = [{"function_declarations": FUNCTION_DECLARATIONS}]


def dispatch(api: BCSAPI, name: str, args: Dict[str, Any]) -> Dict[str, Any]:
    """Execute a Gemini function call and return a JSON-able result."""
