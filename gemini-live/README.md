...
```bash
docker build --platform linux/amd64 -f Dockerfile.bench -t <registry>/riley-gemini-live:v1 .
docker push <registry>/riley-gemini-live:v1
```

Import `.veris/veris.yaml` as a managed image candidate, set `image.path` to
`/voice` and `image.health_path` to `/health`, and add `GEMINI_API_KEY` as a secret
