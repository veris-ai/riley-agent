...
it lands in the graded trace. The Grok realtime session runs the LLM in xAI's
cloud and the tools execute in-process here, so tool calls never reach the
actor transcript;
...
logger = logging.getLogger("riley-grok-voice")
