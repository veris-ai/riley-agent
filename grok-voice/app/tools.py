...
The schemas are wire-format entries for the Grok realtime session's `tools`
array (OpenAI Realtime-compatible). `dispatch()` maps a realtime tool call to
the corresponding `BCSAPI` operation and returns a JSON-serializable result.
"""
...
def dispatch(api: BCSAPI, name: str, args: Dict[str, Any]) -> Dict[str, Any]:
    """Execute a realtime function call and return a JSON-able result."""
