...
it lands in the graded trace. Retell executes tools as HTTP webhooks to /tool
on this server, so the calls never reach the actor transcript; without this
the grader can't see them and may flag real, completed actions as fabricated.
``SIMULATION_ID`` and ``ENGINE_URL`` are set by the sandbox; outside a
simulation this no-ops.
"""
...
logger = logging.getLogger("riley-retell")
