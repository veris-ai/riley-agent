...
The schemas are wire-format entries for the AssemblyAI Voice Agent session
`tools` array. `dispatch()` maps a `tool.call` event to the corresponding
...
TOOLS: list[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            "Update the delivery status (requested/mailed/delivered) of a card "
            "replacement."
        ),
        "parameters": {
            "type": "object",
...
def dispatch(api: BCSAPI, name: str, args: Dict[str, Any]) -> Dict[str, Any]:
    """Execute a Voice Agent tool call and return a JSON-able result."""
